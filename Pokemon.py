...
def raffle(player_profile):
    ...
    if potion == 1:
        print("{} te has ganado una poción de vida, FELICIDADES!!!\n".format(player_profile["player_name"]))
        player_profile["health_potion"] += 1
    else:
        print("Lo siento {} lastimosamente no te tocó la poción de vida, vuelve a intentarlo en la siguiente ronda!\n"
              .format(player_profile["player_name"]))
...
def main():
    ...
    print("\nAntes de iniciar la partida aqui van un par de instrucciones:\n"
          "* Para escoger los pokemons y los ataques de los mismos tienes que usar los indices del uno al cero.\n"
          "* Tendras tres pokemons los cuales podras curar despues antes de cada combate.\n"
          "* Si pierdes un combate, el pokemon con el cual estas luchando será eliminado de tu inventario.\n"
          "* Una vez el número de pokemons en tu inventario sea igual a 0 la partida se terminará.\n"
          "* Al final de cada partida tendras la cantidad de combates que ganaste.")

    input("\nSi entendiste todo a la perfeción presiona [ENTER] para continuar...")
    os.system("cls")
